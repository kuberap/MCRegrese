...
def train_evaluate_model(config, data_dir=None):
    ...
    activations = [config["act1"],config["act2"],config["act3"]]
    layers = config["layers"]
    model = get_model(units=units[0:layers], dropouts=dropouts[0:layers], activations=activations[0:layers], in_dim=IN_DIM, out_dim=OUT_DIM)
