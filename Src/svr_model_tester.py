...
TRAINING_RESULTS_NAME=f"SVR-TRAIN.csv"
TESTING_RESULTS_NAME=f"SVR-TEST.csv"
...
if __name__=="__main__":
    ...
    y_hat_train_sc = np.zeros(y_train_sc.shape)
    y_hat_test_sc = np.zeros(y_test_sc.shape)
    print(y_hat_train_sc.shape)
    ...
    print(to_predict_sc)
    y_hat_signal_sc=np.zeros((1, OUT_DIM))
    for component in range(OUT_DIM):
        print(component)
        model = SVR(kernel="rbf", C=100,  epsilon=0.1)
        model.fit(X_train_sc, y_train_sc[:,component])
        # udelej predikce
        y_hat_train_sc[:, component] = model.predict(X_train_sc)
        y_hat_test_sc[:, component] = model.predict(X_test_sc)
        print(model.predict(to_predict_sc))
        y_hat_signal_sc[0, component] = model.predict(to_predict_sc)[0]
