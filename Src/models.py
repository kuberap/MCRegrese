...
#keras.layers.LeakyReLU(alpha=0.01)
    ...
    for u, d, a in zip(units, dropouts, activations):
        model.add(Dense(units=u, activation=a, kernel_regularizer=keras.regularizers.l2(0.0005)))
        model.add(Dropout(d))
    model.add(Dense(units=out_dim, activation="linear")) # bylo linear
    return model



def get_mlp_model_with_dropout_1(in_dim, out_dim, units=[512, 256,128,64], dropouts=[0.1, 0.1, 0.1,0.1], activations=['relu', 'relu', 'relu',"relu"]):
    assert len(units) == len(dropouts), f"Different length of units and dropouts {len(units)}:{len(dropouts)}"
    assert len(units) == len(activations), f"Different length of units and activations {len(units)}:{len(activations)}"
    model = models.Sequential()
    model.add(InputLayer(input_shape=(in_dim,)))
